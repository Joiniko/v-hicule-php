            ...
            foreach ($rules as $rule) {
                ...
                if ($rule === 'file' && isset($_FILES[$field])) {
                    if ($_FILES[$field]['error'] !== UPLOAD_ERR_OK) {
                        $errors[$field][] = "Le fichier $field est obligatoire.";
                    }
                }
                ...
                if (strpos($rule, 'type:') === 0 && isset($_FILES[$field])) {
                    ...
                    if (!in_array($fileType, $allowedTypes)) {
                        $errors[$field][] = "Le fichier $field doit être de type " . implode(', ', $allowedTypes) . ".";
                    }
                }
                
                if (strpos($rule, 'max_size:') === 0 && isset($_FILES[$field])) {
                    $maxSize = (int)explode(':', $rule)[1] * 1024 * 1024;
                    if ($_FILES[$field]['size'] > $maxSize) {
                        $errors[$field][] = "Le fichier $field ne doit pas dépasser " . ($maxSize / (1024 * 1024)) . " Mo.";
                    }
                }
